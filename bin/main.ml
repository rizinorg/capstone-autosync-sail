...
let get_generator_comment () sail_hash =
  ...
  ^ String.concat "\n * "
      [
        "THIS IS AUTOGENERATED CODE. DO NOT MODIFY THIS CODE MANUALLY.";
...
let write_c_file ?(additional_includes = []) out_directory sail_hash name code =
  ...
  let define_name = String.uppercase_ascii name_no_dots in
  Printf.fprintf oc "%s" (get_generator_comment () sail_hash);
  Printf.fprintf oc "#ifndef %s_H\n" define_name;
  Printf.fprintf oc "#define %s_H\n\n" define_name;
  Printf.fprintf oc "%s" include_string;
  Printf.fprintf oc "%s\n" additional_includes_string;
